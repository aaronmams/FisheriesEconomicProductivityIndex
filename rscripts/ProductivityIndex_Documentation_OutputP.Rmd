---
title: "Productivity Index - Output - By Price"
...
We need time series data for the value of all species ($V_{t}$/Total), value of all species in a category (i) ($V_{i=1}$; e.g., Finfish), value for an individual species in a category (i) ($V_{i=1, s=n}$; e.g., Salmon and Summer Flounder), the quantity of individual species in a category (i) ($Q_{i=1, s=n}$; e.g., Salmon and Flounder):


#### Edit Data

Here we summate the total value for each category and total V for all categories because there may be instances where these values may not be the sum of their parts (though they are here). The caluclation Price Index aims to deal with this potiental issue. 
...
###Price for each species ($P_{t,i,s}$; e.g., Salmon and Flounder)
...
`r ifelse(showresults == TRUE, "We use this *ImplicitQuantityOutput.p* function to calculate the Implicit Quanity Output at Species and category Level:", "")` 
...
```{r, echo = echoTF(FALSE, code)}
temp0<-temp[,paste0("Q", NameBaseTotal)]
temp0 %>%
    knitr::kable(row.names = T, booktabs = T)
```
...
```{r, echo = echoTF(FALSE, code)}
temp0<-temp[,paste0("QI", NameBaseTotal)]
temp0 %>%
    knitr::kable(row.names = T, booktabs = T)
```
...
```{r, echo = echoTF(FALSE, code)}
temp0<-temp[,paste0("QEI", NameBaseTotal)]
temp0 %>%
    knitr::kable(row.names = T, booktabs = T)
```
...
`r ifelse(showresults == TRUE, "We use this *ImplicitQuantityOutput.p* function to calculate the Implicit Quanity Output at Fishery Level:", "")` 
