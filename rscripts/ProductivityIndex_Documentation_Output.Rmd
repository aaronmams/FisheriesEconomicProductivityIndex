...
###Price for each category ($P_{i,t}$; Finfish & others and Shellfish)

$$P_{i,t} = P_{i,t-1}*[1+ln(\frac{P_{s,i,t}}{P_{s,i,t-1}})]$$
...
# 
# PC0<-temp[,names(temp) %in% paste0("PC", NameBasecategory)]
# 
...
# names(temp)[ncol(temp)]<-paste0("P", NameBasecategory)
# # temp$P1_Finfish<-tempP
```
...
###Price Indexes for each category ($PI_{i,t}$; Finfish & others and Shellfish) 

We calculate the price index first by comparing by multiplying the previous years $PI_{i,t-1}$ by that year's price change $PC_{i,t}$, where the PI of the first year $PI_{i, t=firstyear} = 1$

$$PI_{i,t} = PI_{i,t-1}*(1+PC_{i,t})$$
$$PI_{i, t_{first year}} = 1$$

Then, to change the price (calulated later) into base year dollars, we use the following equation: 

$$PI_{i,t} = PI_{i,t}/PI_{i, t = baseyear}$$


In this example, we'll decide that the base year is `r baseyr`. Notice that the $PI_{i,t=baseyr} = 1$ 
...
```{r, echo  = TRUE}
  ...
  if (sum(temp0[,paste0("V", NameBasecategory, "_Check")] %in% 
    ...
    print("When back calculated, V_{i,t} did not equal P_{i,t} * Q_{i,t}")
  }
```
...
```{r, echo  = TRUE}
  ...
  if (sum(temp0[,paste0("Q", NameBasecategory, "_Check")] %in% 
    ...
    print("When back calculated, Q_{i,t} did not equal V_{i,t}/P_{i,t}")
  }
```
...
```{r, echo  = TRUE}
temp$PC0_0Total<-rowSums(cbind(pricechange(R0 = temp$R1_0Finfish, P0 = temp$P1_0Finfish), 
                            pricechange(R0 = temp$R2_0Shellfish, P0 = temp$P2_0Shellfish)), 
                       na.rm = T)
```
...
PC0<-temp$PC0_0Total #this is equal to ln(P_it/P_it-1)
...
```{r, echo  = FALSE}
# temp$PI0_0Total<-temp$P0_0Total/temp$P0_0Total[rownames(temp) %in% baseyr]
...
```{r, echo  = TRUE}
  ...
  print("When back calculated, V_t did not equal P_t * Q_t")
  }
```
...
```{r, echo  = TRUE}
  ...
  print("When back calculated, Q_t did not equal V_t/P_t")
  }
```
...
### Graph 1: Price Index
In theory, $PI$ should be negative after the baseyear and positive after the base year, but because this data was fabricated without thinking of this, we don't see that here. The index value for the base year is =1, however. 
...
We can do the structuring work in a function
```{r, echo  = TRUE}
# This function structures the data for the Implicit Quantity function
# print(EditCommData)
```
